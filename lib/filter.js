 ...
 * @param {Boolean} data.noMaxSize - Disable maximum size checks
 * @constructor
 */
function Filter(arg) {
  ...
  if (typeof(arg) === 'object') {
    ...
    if (!arg.noMaxSize && arg.vData.length > Filter.MAX_BLOOM_FILTER_SIZE * 8) {
    ...
    this.noMaxSize = arg.noMaxSize || false;
  } else {
...
Filter.prototype.toObject = function toObject() {
  return {
    ...
    nTweak: this.nTweak,
    nFlags: this.nFlags,
    noMaxSize: this.noMaxSize
  };
};

Filter.create = function create(elements, falsePositiveRate, nTweak, nFlags, noMaxSize) {
...
Filter.prototype.inspect = function inspect() {
  var self = this;
  function max() {
    return self.noMaxSize ? ' noMaxSize:true' : '';
  }
  function formatData() {
    var data = self.vData.slice(0, 32);
    var end = self.vData.length > 32 ? '...' : '';
    return data.toJSON().data + end;
  }
  return '<BloomFilter:' +
    formatData() + ' nHashFuncs:' +
